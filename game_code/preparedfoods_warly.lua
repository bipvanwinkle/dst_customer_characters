local foods =
{
  ...
  nightmarepie =
  {
    test = function(cooker, names, tags) return names.nightmarefuel and names.nightmarefuel == 2 and
          (names.potato or names.potato_cooked) and (names.onion or names.onion_cooked)
    end,
    priority = 30,
    ...
    oneatenfn = function(inst, eater)
      if eater.components.sanity ~= nil and eater.components.health ~= nil and eater.components.oldager == nil then
        ...
        eater.components.health:DoDelta(sanity_percent * eater.components.health.maxhealth -
          eater.components.health.currenthealth, nil, "nightmarepie")
      end
    end,
  ...
  voltgoatjelly =
  {
    test = function(cooker, names, tags) return (names.lightninggoathorn) and (tags.sweetener and tags.sweetener >= 2)
          and not tags.meat
    end,
    priority = 30,
  ...
  glowberrymousse =
  {
    test = function(cooker, names, tags) return (
          names.wormlight or (names.wormlight_lesser and names.wormlight_lesser >= 2)) and
          (tags.fruit and tags.fruit >= 2
  ...
  frogfishbowl =
  {
    test = function(cooker, names, tags) return (
          (names.froglegs and names.froglegs >= 2) or (names.froglegs_cooked and names.froglegs_cooked >= 2) or
              (names.froglegs and names.froglegs_cooked)) and tags.fish and tags.fish >= 1 and not tags.inedible
    end,
    priority = 30,
  ...
  dragonchilisalad =
  {
    test = function(cooker, names, tags) return (names.dragonfruit or names.dragonfruit_cooked) and
          (names.pepper or names.pepper_cooked) and not tags.meat and not tags.inedible and not tags.egg
    end,
    priority = 30,
    foodtype = FOODTYPE.VEGGIE,
    health = -TUNING.HEALING_SMALL,
  ...
  {
    test = function(cooker, names, tags) return (
          (names.potato and names.potato >= 2) or (names.potato_cooked and names.potato_cooked >= 2) or
              (names.potato and names.potato_cooked)) and tags.egg and not tags.meat and not tags.inedible
    end,
    priority = 30,
    foodtype = FOODTYPE.VEGGIE,
    health = TUNING.HEALING_MED,
  ...
  bonesoup =
  {
    test = function(cooker, names, tags) return names.boneshard and names.boneshard == 2 and
          (names.onion or names.onion_cooked) and (tags.inedible and tags.inedible < 3)
    end,
    priority = 30,
    foodtype = FOODTYPE.MEAT,
  ...
  moqueca =
  {
    test = function(cooker, names, tags) return tags.fish and (names.onion or names.onion_cooked) and
          (names.tomato or names.tomato_cooked) and not tags.inedible
    end,
    priority = 30,
    foodtype = FOODTYPE.MEAT,
