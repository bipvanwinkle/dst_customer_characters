...
local function CanCastFishingNetAtPoint(thrower, target_x, target_z)
  ...
  if TheWorld.Map:IsOceanAtPoint(target_x, 0, target_z) and
      VecUtil_LengthSq(target_x - thrower_x, target_z - thrower_z) > min_throw_distance * min_throw_distance then
    return true
  end
  return false
end
...
local function Row(inst, doer, pos, actions)
  ...
  elseif doer ~= nil and not doer:HasTag("is_row_failing") then
    ...
    elseif not is_controller_attached then
      ...
      local found_water = not map:IsVisualGroundAtPoint(test_x, 0, test_z) and
          map:GetPlatformAtPoint(test_x, test_z) == nil
      if found_water then
  ...
  if inst ~= doer and (doer.CanExamine == nil or doer:CanExamine()) then
    ...
    if plantinspector and ((inst.GetPlantRegistryKey and inst.GetResearchStage) or inst.GetFertilizerKey) then
      ...
      if (not TheNet:IsDedicated() and doer == ThePlayer) then
        if (
            inst:HasTag("plantresearchable") and
...
local function CheckRowOverride(doer, target)
  ...
  if target ~= nil then
    ...
    if target:HasTag("overriderowaction") and
        math.min(dist_to_target, boat_dist_to_target) < TUNING.OVERRIDE_ROW_ACTION_DISTANCE then
      return true
    end
  end
...
local COMPONENT_ACTIONS =
{
  SCENE = --args: inst, doer, actions, right
  {
    ...
    farmplanttendable = function(inst, doer, actions)
      if inst:HasTag("tendable_farmplant") and not inst:HasTag("fire") and not inst:HasTag("smolder") and
          not doer:HasTag("mime") then
    ...
    inspectable = function(inst, doer, actions)
      if inst ~= doer and
          ...
          (
          doer.sg == nil or
              (doer.sg:HasStateTag("idle") and not doer.sg:HasStateTag("moving") or doer.sg:HasStateTag("channeling")))
          and
    ...
    kitcoon = function(inst, doer, actions, right)
      if right then
        if inst.replica.follower ~= nil and inst.replica.follower:GetLeader() == doer then
          if doer:HasTag("near_kitcoonden") and
              FindEntity(inst, TUNING.KITCOON_NEAR_DEN_DIST, nil, { "kitcoonden" }) ~= nil then
    ...
    mast = function(inst, doer, actions, right)
      ...
      if inst:HasTag("sailraised") then
        if not doer:HasTag("is_furling") then
          ...
          if doer.AnimState:IsCurrentAnimation("pull_big_pre") or doer.AnimState:IsCurrentAnimation("pull_big_lag") or
              doer.AnimState:IsCurrentAnimation("pull_big_loop") then
            return table.insert(actions, ACTIONS.LOWER_SAIL_FAIL)
          elseif doer.AnimState:IsCurrentAnimation("pull_small_loop") or
              doer.AnimState:IsCurrentAnimation("pull_small_pre") then
            return table.insert(actions, ACTIONS.LOWER_SAIL_BOOST)
          end
        end
    ...
    boatrotator = function(inst, doer, actions, right)
      ...
      if boat and boat:HasTag("boat") and boat.components.boatringdata and not inst:HasTag("fire") and
        ...
        if (doer.replica.inventory ~= nil and doer.replica.inventory:IsHeavyLifting()) and
          ...
          if item ~= nil then
            for _, v in pairs(TROPHYSCALE_TYPES) do
              if inst:HasTag("trophyscale_" .. v) and item:HasTag("weighable_" .. v) and
                  item.replica.inventoryitem ~= nil and item.replica.inventoryitem:IsGrandOwner(doer) then
    ...
    groomer = function(inst, doer, actions, right)
      if inst:HasTag("groomer") and not inst:HasTag("fire") and not inst:HasTag("burnt") and not inst:HasTag("hitcher")
          and right then
        table.insert(actions, ACTIONS.CHANGEIN)
      end
    end,
    ...
    hitcher = function(inst, doer, actions, right)
      if inst:HasTag("hitcher") and not inst:HasTag("fire") and not inst:HasTag("burnt") and
          not inst:HasTag("hitcher_locked") then
        table.insert(actions, ACTIONS.HITCHUP)
      end

      -- [TODO] this needs to confirm that beefalo is the owners beef.
      if not inst:HasTag("hitcher") and not inst:HasTag("fire") and not inst:HasTag("burnt") and
          not inst:HasTag("hitcher_locked") and not right then
    ...
    quagmire_tappable = function(inst, doer, actions, right)
      if not inst:HasTag("tappable") and not inst:HasTag("fire") then
        if right then
          ...
          table.insert(actions,
            inst:HasTag("tapped_harvestable") and doer.replica.inventory:EquipHasTag("CHOP_tool") and ACTIONS.HARVEST or
            ACTIONS.TAPTREE)
  ...
  USEITEM = --args: inst, doer, target, actions, right
  {
              ...
              for i, v2 in ipairs(v.types) do
                if inst:HasTag("edible_" .. v2) then
                  if iscritter or ishandfed then
                    if (target.replica.follower ~= nil and target.replica.follower:GetLeader() == doer) or
                        target:HasTag("fedbyall") then
                  ...
                  elseif target:HasTag("player") then
                    if TheNet:GetPVPEnabled() or
                        ...
                        (inst:HasTag("spoiled") and target:HasTag("ignoresspoilage") and not
                            (inst:HasTag("badfood") or inst:HasTag("unsafefood"))) or
                        not -- ignoresspoilage still checks for unsage foods
    ...
    inventoryitem = function(inst, doer, target, actions, right)
      ...
      elseif inventoryitem ~= nil
        ...
        if not (GetGameModeProperty("non_item_equips") and inst.replica.equippable ~= nil) and
            (
            (
                inst.prefab ~= "spoiled_food" and inst:HasTag("quagmire_stewable") and target:HasTag("quagmire_stewer")
                    and target.replica.container:IsOpenedBy(doer)) or
      ...
      elseif target.replica.constructionsite ~= nil then
        if not (GetGameModeProperty("non_item_equips") and inst.replica.equippable ~= nil) and
          ...
          table.insert(actions,
            target.replica.constructionsite:IsBuilder(doer) and ACTIONS.BUNDLESTORE or ACTIONS.CONSTRUCT)
        end
    ...
    itemweigher = function(inst, doer, target, actions)
      for _, v in pairs(TROPHYSCALE_TYPES) do
        if inst:HasTag("trophyscale_" .. v) and target:HasTag("weighable_" .. v) and target.replica.inventoryitem ~= nil
    ...
    lighter = function(inst, doer, target, actions)
      if target:HasTag("canlight") and
          not ((target:HasTag("fueldepleted") and not target:HasTag("burnableignorefuel")) or target:HasTag("INLIMBO")) then
    ...
    pocketwatch = function(inst, doer, target, actions)
      if inst:HasTag("pocketwatch_inactive") and doer:HasTag("pocketwatchcaster") and inst.pocketwatch_CanTarget ~= nil
          and inst:pocketwatch_CanTarget(doer, target) then
        ...
        for k, v in pairs(MATERIALS) do
          if target:HasTag("repairable_" .. v) then
            if (inst:HasTag("work_" .. v) and target:HasTag("workrepairable"))
                ...
                or
                (
                inst:HasTag("freshen_" .. v) and
                    (target:HasTag("fresh") or target:HasTag("stale") or target:HasTag("spoiled")))
    ...
    sleepingbag = function(inst, doer, target, actions)
      if (doer == target and doer:HasTag("player") and not doer:HasTag("insomniac") and not inst:HasTag("hassleeper"))
          and
          (not inst:HasTag("spiderden") or doer:HasTag("spiderwhisperer")) then
        table.insert(actions, ACTIONS.SLEEPIN)
      end
    ...
    stackable = function(inst, doer, target, actions)
      if inst.prefab == target.prefab and inst.AnimState:GetSkinBuild() == target.AnimState:GetSkinBuild() and
          --inst.skinname == target.skinname (this does not work on clients, so we're going to use the AnimState hack instead)
    ...
    summoningitem = function(inst, doer, target, actions, right)
      if not target.inlimbo and target.replica.follower ~= nil and target.replica.follower:GetLeader() == doer and
          doer:HasTag("ghostfriend_summoned") and target:HasTag("abigail") then
    ...
    useabletargeteditem = function(inst, doer, target, actions)
      if target ~= nil then
        if (target.prefab ~= nil and inst:HasTag(target.prefab .. "_targeter") and not inst:HasTag("inuse_targeted"))
            or
            (inst.UseableTargetedItem_ValidTarget ~= nil and inst.UseableTargetedItem_ValidTarget(inst, target, doer)
            ) then
    ...
    weapon = function(inst, doer, target, actions, right)
      ...
      elseif not right and
          ...
          (
          inst:HasTag("projectile") or inst:HasTag("rangedweapon") or
              not (doer.replica.rider ~= nil and doer.replica.rider:IsRiding())) and
    ...
    winter_treeseed = function(inst, doer, target, actions)
      if target:HasTag("winter_treestand") and
          not (target:HasTag("fire") or target:HasTag("smolder") or target:HasTag("burnt")) then
        table.insert(actions, ACTIONS.PLANT)
      end
    end,
    ...
    bedazzler = function(inst, doer, target, actions)
      if doer:HasTag("spiderwhisperer") and target:HasTag("spiderden") and target:HasTag("bedazzleable") and
          not target:HasTag("bedazzled") then
  ...
  POINT = --args: inst, doer, pos, actions, right, target
  {
    blinkstaff = function(inst, doer, pos, actions, right, target)
      ...
      if right and (TheWorld.Map:IsAboveGroundAtPoint(x, y, z) or TheWorld.Map:GetPlatformAtPoint(x, z) ~= nil) and
          ...
          not doer:HasTag("rotatingboat") then
    ...
    deployable = function(inst, doer, pos, actions, right, target)
      if right and inst.replica.inventoryitem ~= nil then
        ...
        elseif inst.replica.inventoryitem:CanDeploy(pos, nil, doer,
          (doer.components.playercontroller ~= nil and doer.components.playercontroller.deployplacer ~= nil) and
          doer.components.playercontroller.deployplacer.Transform:GetRotation() or 0) then
    ...
    spellcaster = function(inst, doer, pos, actions, right, target)
      if right then
        ...
        if inst:HasTag("castonpoint") then
          ...
          if TheWorld.Map:IsAboveGroundAtPoint(px, py, pz, cast_on_water) and not TheWorld.Map:IsGroundTargetBlocked(pos)
        ...
        elseif cast_on_water then
          local px, py, pz = pos:Get()
          if TheWorld.Map:IsOceanAtPoint(px, py, pz, false) and not TheWorld.Map:IsGroundTargetBlocked(pos) and
              not doer:HasTag("steeringboat") and not doer:HasTag("rotatingboat") then
            table.insert(actions, ACTIONS.CASTSPELL)
          end
        end
      end
  ...
  EQUIPPED = --args: inst, doer, target, actions, right
  {
    ...
    complexprojectile = function(inst, doer, target, actions, right)
      if right and
          not (doer.components.playercontroller ~= nil and doer.components.playercontroller.isclientcontrollerattached)
          and
    ...
    lighter = function(inst, doer, target, actions, right)
      if right and target:HasTag("canlight") and
          not ((target:HasTag("fueldepleted") and not target:HasTag("burnableignorefuel")) or target:HasTag("INLIMBO")) then
        table.insert(actions, ACTIONS.LIGHT)
      end
    end,
    ...
    spellcaster = function(inst, doer, target, actions, right)
      if right and not target:HasTag("nomagic") and (
          inst:HasTag("castontargets") or
              (
              inst:HasTag("castonrecipes") and AllRecipes[target.prefab] ~= nil and
              ...
              (target:HasTag("locomotor") and (
                  inst:HasTag("castonlocomotors") or
                      (
                      inst:HasTag("castonlocomotorspvp") and
                          (
                          target == doer or TheNet:GetPVPEnabled() or
                              not (target:HasTag("player") and doer:HasTag("player"))))
                  )) or
              (
              inst:HasTag("castonworkable") and
                  (
                  target:HasTag("CHOP_workable") or target:HasTag("DIG_workable") or target:HasTag("HAMMER_workable") or
  ...
  INVENTORY = --args: inst, doer, actions, right
  {
    ...
    oceanfishingtackle = function(inst, doer, actions, right)
      if doer.replica.inventory ~= nil and not doer.replica.inventory:IsHeavyLifting() then
        ...
        if rod ~= nil and rod.replica.container ~= nil and rod.replica.container:IsOpenedBy(doer) and
            rod:HasTag("accepts_oceanfishingtackle") and rod.replica.container:CanTakeItemInSlot(inst) then
    ...
    pocketwatch = function(inst, doer, actions)
      if inst:HasTag("pocketwatch_inactive") and doer:HasTag("pocketwatchcaster") and
          inst:HasTag("pocketwatch_castfrominventory") then
        if not (doer.replica.rider ~= nil and doer.replica.rider:IsRiding()) or inst:HasTag("pocketwatch_mountedcast") then
          table.insert(actions, ACTIONS.CAST_POCKETWATCH)
        end
      end
    ...
    reloaditem = function(inst, doer, actions, right)
      if doer.replica.inventory ~= nil and not doer.replica.inventory:IsHeavyLifting() then
        ...
        if hand_item ~= nil and hand_item.replica.container ~= nil and hand_item.replica.container:IsOpenedBy(doer) and
            hand_item.replica.container:CanTakeItemInSlot(inst) then
          table.insert(actions, ACTIONS.CHANGE_TACKLE)
        end
      end
    end,
...
local function ModComponentWarning(self, modname)
  print("ERROR: Mod component actions are out of sync for mod " ..
    (modname or "unknown") ..
...
function EntityScript:CollectActions(actiontype, ...)
  ...
  if t == nil then
    print("Action type", actiontype,
      "doesn't exist in the table of component actions. Is your component name correct in AddComponentAction?")
    return
  end
  for i, v in ipairs(self.actioncomponents) do
