...
function c_countallprefabs()
  ...
  print(string.format("There are %d different prefabs in the world, %d total (and %d unknown)", GetTableSize(counted),
    total, unk))
end
...
function c_maintainhealth(player, percent)
  ...
  if player ~= nil and player.components.health ~= nil then
    ...
    player.debug_maintainhealthtask = player:DoPeriodicTask(3,
      function(inst) inst.components.health:SetPercent(percent or 1) end)
  end
end
...
function c_maintainsanity(player, percent)
  ...
  if player ~= nil and player.components.sanity ~= nil then
    ...
    player.debug_maintainsanitytask = player:DoPeriodicTask(3,
      function(inst) inst.components.sanity:SetPercent(percent or 1) end)
  end
end
...
function c_maintainhunger(player, percent)
  ...
  if player ~= nil and player.components.hunger ~= nil then
    ...
    player.debug_maintainhungertask = player:DoPeriodicTask(3,
      function(inst) inst.components.hunger:SetPercent(percent or 1) end)
  end
end
...
function c_maintaintemperature(player, temp)
  ...
  if player ~= nil and player.components.temperature ~= nil then
    ...
    player.debug_maintaintemptask = player:DoPeriodicTask(3,
      function(inst) inst.components.temperature:SetTemperature(temp or 25) end)
  end
end
...
function c_maintainmoisture(player, percent)
  ...
  if player ~= nil and player.components.moisture ~= nil then
    ...
    player.debug_maintainmoisturetask = player:DoPeriodicTask(3,
      function(inst) inst.components.moisture:SetPercent(percent or 0) end)
  end
end
...
function c_shellsfromtable(song, startpos, placementfn, spacing_multiplier, out_of_range_mode)
  ...
  if out_of_range_mode ~= "AUTO_TRANSPOSE" and out_of_range_mode ~= "OMIT" and out_of_range_mode ~= "TRUNCATE" and
      out_of_range_mode ~= "TERMINATE" then
    ...
    if song_semitone_range < allowed_semitone_range.upper - allowed_semitone_range.lower then
      ...
      for i, shell_data in ipairs(shells_to_spawn) do
        ...
        if shell_prefab == nil or shell_prefab == "" then
          print("Error: Auto-transposition failed for semitone " ..
            shell_data.semitone .. " at index " .. i .. ".\nRemoving all spawned shell instances.")

          CleanupShells(spawned_shells)
          return nil
        else
          table.insert(spawned_shells,
            SpawnShell(shell_prefab, shell_data, transposed_semitone - semitone_shell_start[shell_prefab]))
        end
      end
    else
  ...
  elseif out_of_range_mode == "OMIT" then
    for i, shell_data in ipairs(shells_to_spawn) do
      ...
      if shell_prefab ~= nil and shell_prefab ~= "" then
        table.insert(spawned_shells,
          SpawnShell(shell_prefab, shell_data, shell_data.semitone - semitone_shell_start[shell_prefab]))
      else
        print("Warning: Omitting shell at index " ..
          i .. " semitone " .. shell_data.semitone .. ".\nSemitone outside shell tonal range 37(C3) - 72(B5).")
      end
    end
  elseif out_of_range_mode == "TRUNCATE" then
    for i, shell_data in ipairs(shells_to_spawn) do
      ...
      if shell_prefab ~= nil and shell_prefab ~= "" then
        ...
        print("Discontinuing shell spawning at index " ..
    ...
    for i, shell_data in ipairs(shells_to_spawn) do
      ...
      if shell_prefab ~= nil and shell_prefab ~= "" then
        ...
        print("Terminating shell spawning at index " ..
          i ..
          ".\nSemitone " ..
          shell_data.semitone .. " outside shell tonal range 37(C3) - 72(B5).\nRemoving all spawned shell instances.")
