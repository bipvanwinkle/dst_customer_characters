...
function Entity:AddNetwork()
  ...
  for _, modname in pairs(ModManager:GetServerModsNames()) do
    inst.actionreplica.modactioncomponents[modname] = net_smallbytearray(guid, "modactioncomponents" .. modname,
      "modactioncomponentsdirty" .. modname)
  end
  ...
  if not TheWorld.ismastersim then
    ...
    for _, modname in pairs(ModManager:GetServerModsNames()) do
      inst:ListenForEvent("modactioncomponentsdirty" .. modname,
        function(inst) OnModActionComponentsDirty(inst, modname) end)
    end
  end
end
  ...
  if self.entity:HasTag("player") then
    ...
    if platform then
      ...
      if rx ~= rx or ry ~= ry or rz ~= rz then
        print("EntityScript:GetSaveRecord error saving position: ", self.prefab, rx, ry, rz, ":", x, y, z, ":", px, py,
          pz)
...
function EntityScript:GetBasicDisplayName()
  return (self.displaynamefn ~= nil and self:displaynamefn())
      ...
      or
      (self.name_author_netid ~= nil and ApplyLocalWordFilter(self.name, TEXT_FILTER_CTX_CHAT, self.name_author_netid))
      -- this is more lika a TEXT_FILTER_CTX_NAME but its all user input (eg, naming a beefalo) so lets go with TEXT_FILTER_CTX_CHAT
      or self.name
end
...
function EntityScript:GetIsWet()
  ...
  return self:HasTag("wet") or TheWorld.state.iswet or
...
function EntityScript:PushBufferedAction(bufferedaction)
  ...
  if bufferedaction.action == ACTIONS.WALKTO then
    ...
    self.bufferedaction = nil
  elseif bufferedaction.action.instant then
    if bufferedaction.target ~= nil and bufferedaction.target.Transform ~= nil and
        (self.sg == nil or self.sg:HasStateTag("canrotate")) then
...
function EntityScript:PerformBufferedAction()
  if self.bufferedaction then
    ...
    local action_theme_music = self:HasTag("player") and
        (
        self.bufferedaction.action.theme_music or
            (
            self.bufferedaction.action.theme_music_fn ~= nil and
                self.bufferedaction.action.theme_music_fn(self.bufferedaction)))
