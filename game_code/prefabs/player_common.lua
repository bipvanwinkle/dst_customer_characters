  ...
  return FindEntity(inst, 10,
    function(target)
      return (target.components.combat ~= nil and target.components.combat.target == inst)
          or (
          (
              target:HasTag("monster") and not target:HasTag("companion") and
                  (
                  target.components.follower == nil or target.components.follower:GetLeader() == nil or
                      not target.components.follower:GetLeader():HasTag("player"))
...
local function CanUseTouchStone(inst, touchstone)
  ...
  elseif inst.player_classified ~= nil then
    return touchstone.GetTouchStoneID ~= nil and
        not table.contains(inst.player_classified.touchstonetrackerused:value(), touchstone:GetTouchStoneID())
  else
    return false
  end
end
...
local function GetMoonstormLevel(inst)
  ...
  for _, edge_index in ipairs(node_edges) do
    ...
    if not _active_moonstorm_nodes[other_node_index] then
      ...
      local node1 = { x = TheWorld.topology.flattenedPoints[point_indices[1]][1],
        y = TheWorld.topology.flattenedPoints[point_indices[1]][2] }
      local node2 = { x = TheWorld.topology.flattenedPoints[point_indices[2]][1],
        y = TheWorld.topology.flattenedPoints[point_indices[2]][2] }
...
local function OnGetItem(inst, giver, item)
  ...
  elseif item ~= nil and giver.components.age ~= nil then
    if giver.components.age:GetAgeInDays() >= TUNING.ACHIEVEMENT_HELPOUT_GIVER_MIN_AGE and
        inst.components.age:GetAgeInDays() <= TUNING.ACHIEVEMENT_HELPOUT_RECEIVER_MAX_AGE then
...
local function DropWetTool(inst, data)
  ...
  if tool ~= nil and tool:GetIsWet() and
      math.random() < easing.inSine(TheWorld.state.wetness, 0, .15, inst.components.moisture:GetMaxMoisture()) then
    local projectile =
    data.weapon ~= nil and
...
local function OnStartedFire(inst, data)
  if data ~= nil and data.target ~= nil and data.target:HasTag("structure") and
      not data.target:HasTag("wildfireprotected") then
...
local function DoEffects(pet)
  SpawnPrefab(pet:HasTag("flying") and "spawn_fx_small_high" or "spawn_fx_small").Transform:SetPosition(pet.Transform:
    GetWorldPosition())
end
...
local function ShakeCamera(inst, mode, duration, speed, scale, source_or_pt, maxDist)
  if source_or_pt ~= nil and maxDist ~= nil then
    local distSq = source_or_pt.entity ~= nil and inst:GetDistanceSqToInst(source_or_pt) or
        inst:GetDistanceSqToPoint(source_or_pt:Get())
...
local function SaveForReroll(inst)
  ...
  inst.components.inventory:ForEachItem(function(thing)
    ...
    if thing.components.curseditem then
      ...
      curses[thing.prefab] = curses[thing.prefab] +
          (thing.components.stackable and thing.components.stackable:StackSize() or 1)
      thing:Remove()
    end
  end)
  ...
  local data =
  {
    ...
    maps = inst.player_classified ~= nil and inst.player_classified.MapExplorer ~= nil and
        inst.player_classified.MapExplorer:RecordAllMaps() or nil,
    seamlessplayerswapper = inst.components.seamlessplayerswapper ~= nil and
        inst.components.seamlessplayerswapper:OnSave() or nil,
    curses = curses,
  }
