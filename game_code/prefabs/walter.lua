...
local function startsong(inst)
  ...
  if inst.AnimState:AnimDone() then
    inst:PushEvent("singsong",
      { sound = "dontstarve/characters/walter/song", lines = STRINGS.SONGS.WALTER_GLOMMER_GUTS.lines })
  end
end
...
local function UpdateTreeSanityGain(inst)
  ...
  local num_trees = #
      TheSim:FindEntities(x, y, z, TUNING.WALTER_TREE_SANITY_RADIUS, REQUIRED_TREE_TAGS, EXCLUDE_TREE_TAGS)
...
local function CustomSanityFn(inst, dt)
  local health_drain = (1 - inst.components.health:GetPercentWithPenalty()) * TUNING.WALTER_SANITY_HEALTH_DRAIN *
      inst._sanity_damage_protection:Get()
...
local function CampfireStory_OnNotNight(inst, isnight)
  if not isnight and inst.storyteller ~= nil and inst.storyteller:IsValid() and
      inst.storyteller.components.storyteller ~= nil then
    inst.storyteller.components.storyteller:AbortStory(GetString(inst.storyteller,
