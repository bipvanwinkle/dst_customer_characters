...
function Eater:Eat(food, feeder)
  ...
  if self:PrefersToEat(food) then
    local stack_mult = self.eatwholestack and food.components.stackable ~= nil and food.components.stackable:StackSize()
        or 1
    ...
    if self.custom_stats_mod_fn ~= nil then
      health_delta, hunger_delta, sanity_delta = self.custom_stats_mod_fn(self.inst, health_delta, hunger_delta,
        sanity_delta, food, feeder)
    end
    ...
    if feeder ~= self.inst and self.inst.components.inventoryitem ~= nil then
      ...
      if owner ~= nil and
          (owner == feeder or (owner.components.container ~= nil and owner.components.container:IsOpenedBy(feeder))) then
