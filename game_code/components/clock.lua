...
return Class(function(self, inst)
  ...
  local OnSetMoonPhaseStyle = _ismastersim and function(world, data)
    _moonphasestyle:set((
      (data ~= nil and data.style ~= nil) and MOON_PHASE_STYLES[data.style] or MOON_PHASE_STYLES.default) - 1)
  end or nil
...
--]]
  function self:OnUpdate(dt)
    ...
    if remainingtimeinphase > 0 then
      ...
      local prevseg = numsegsinphase > 0 and
          math.ceil(_remainingtimeinphase:value() / _totaltimeinphase:value() * numsegsinphase) or 0
      local nextseg = numsegsinphase > 0 and math.ceil(remainingtimeinphase / _totaltimeinphase:value() * numsegsinphase)
          or 0
    ...
    if _moonphasedirty then
      ...
      _world:PushEvent("moonphasechanged2",
        { moonphase = MOON_PHASE_NAMES[_moonphase:value()], waxing = _mooniswaxing:value() })
    ...
    for i, v in ipairs(_segs) do
      if _phase:value() == i then
        normtimeinphase = 1 -
            (_totaltimeinphase:value() > 0 and _remainingtimeinphase:value() / _totaltimeinphase:value() or 0)
    ...
    _world:PushEvent("clocktick",
      { phase = PHASE_NAMES[_phase:value()], timeinphase = normtimeinphase, time = elapsedsegs / NUM_SEGS })
  ...
  if _ismastersim then function self:OnLoad(data)
      ...
      _remainingtimeinphase:set(math.min(data.remainingtimeinphase or _totaltimeinphase:value(),
  ...
  function self:Dump()
    ...
    local to_night = _remainingtimeinphase:value() +
        (PHASE_NAMES[_phase:value()] == "day" and _segs[2]:value() or 0) * TUNING.SEG_TIME
    print("Time Until Night:", to_night, to_night / TUNING.SEG_TIME)
  end

  function self:GetDebugString()
    return string.format("%d %s: %2.2f : %2.2f (moon cycle: %d)", _cycles:value() + 1, PHASE_NAMES[_phase:value()],
      _remainingtimeinphase:value(), _segs[_phase:value()]:value(), _mooomphasecycle)
  end
