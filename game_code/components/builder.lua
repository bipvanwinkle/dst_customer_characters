...
local function oningredientmod(self, ingredientmod)
  assert(INGREDIENT_MOD[ingredientmod] ~= nil,
    "Ingredient mods restricted to certain values, see constants.lua INGREDIENT_MOD")
...
local function metafn()
  ...
  for i, v in ipairs(TechTree.BONUS_TECH) do
    ...
    t[string.lower(v) .. "_tempbonus"] = function(self, bonus) self.inst.replica.builder:SetTempTechBonus(string.lower(v)
        , (bonus or 0))
    end
  end
  return t
...
function Builder:OnSave()
  ...
  local hungrypt = self.last_hungry_build_pt and not CheckHungryDistance(self.inst, self.last_hungry_build_pt) and
      { x = self.last_hungry_build_pt.x, y = self.last_hungry_build_pt.y, z = self.last_hungry_build_pt.z } or nil
...
function Builder:EvaluateTechTrees()
  ...
  if self.override_current_prototyper then
    if self.override_current_prototyper:IsValid()
        ...
        self.override_current_prototyper.components.prototyper.restrictedtag == nil or
            self.inst:HasTag(self.override_current_prototyper.components.prototyper.restrictedtag))
  ...
  for i, v in ipairs(ents) do
    if v.components.prototyper ~= nil and
        (v.components.prototyper.restrictedtag == nil or self.inst:HasTag(v.components.prototyper.restrictedtag)) then
  ...
  if not prototyper_active then
    for i, v in ipairs(TechTree.AVAILABLE_TECH) do
      ...
      self.accessible_tech_trees[v] = (self[string.lower(v) .. "_tempbonus"] or 0) +
...
function Builder:UsePrototyper(prototyper)
  if prototyper ~= nil then
    if not prototyper:HasTags(PROTOTYPER_TAGS)
        ...
        or
        (
        prototyper.components.prototyper ~= nil and prototyper.components.prototyper.restrictedtag ~= nil and
            not self.inst:HasTag(prototyper.components.prototyper.restrictedtag))
    then
  ...
  for item, ents in pairs(ingredients) do
    for k, v in pairs(ents) do
      table.insert(wetness,
        {
          wetness = k.components.inventoryitem ~= nil and k.components.inventoryitem:GetMoisture() or
...
function Builder:HasCharacterIngredient(ingredient)
  if ingredient.type == CHARACTER_INGREDIENT.HEALTH then
    if self.inst.components.health ~= nil then
      ...
      local amount_required = self.inst:HasTag("health_as_oldage") and
          math.ceil(ingredient.amount * TUNING.OLDAGE_HEALTH_SCALE) or ingredient.amount
...
function Builder:MakeRecipe(recipe, pt, rot, skin, onsuccess)
  if recipe ~= nil and not self.inst.sg:HasStateTag("drowning") then -- TODO(JBK): Check if "drowning" can be replaced with "busy" instead with no side effects.
    ...
    if self:IsBuildBuffered(recipe.name) or self:HasIngredients(recipe) then
      ...
      local buffaction = BufferedAction(self.inst, nil, ACTIONS.BUILD, nil, pt or self.inst:GetPosition(), recipe.name,
        recipe.build_distance, nil, rot)
...
function Builder:DoBuild(recname, pt, rotation, skin)
  ...
  if recipe ~= nil and (self:IsBuildBuffered(recname) or self:HasIngredients(recipe)) then
    ...
    if self.inst:HasTag("hungrybuilder") and not self.inst.sg:HasStateTag("slowaction") and
        not self.inst.sg:HasStateTag("giving") then
      local t = GetTime()
      local hasTimeExpired = self.last_hungry_build == nil or
          t > self.last_hungry_build + TUNING.HUNGRY_BUILDER_RESET_TIME
      local hasPlayerMoved = self.last_hungry_build_pt == nil or
          CheckHungryDistance(self.inst, self.last_hungry_build_pt)
...
function Builder:MakeRecipeFromMenu(recipe, skin)
  if self:HasIngredients(recipe) then
        ...
        self:MakeRecipe(recipe, nil, nil, ValidateRecipeSkinRequest(self.inst.userid, recipe.product, skin),
          function()
            ...
            if self.freebuildmode then
              ...
              if not table.contains(self.recipes, recipe.name) and
                  CanPrototypeRecipe(recipe.level, self.accessible_tech_trees) then
    ...
    for i, ing in ipairs(recipe.ingredients) do
      ...
      if ing_recipe ~= nil and
          not
          self.inst.components.inventory:Has(ing.type, math.max(1, RoundBiasedUp(ing.amount * self.ingredientmod)), true)
          and self:HasIngredients(ing_recipe) then
        --Need to determine this NOW before calling async MakeRecipe
        ...
        if self:KnowsRecipe(ing_recipe) then
          self:MakeRecipe(ing_recipe, nil, nil, ValidateRecipeSkinRequest(self.inst.userid, ing_recipe.product, nil),
            function()
              ...
              if self.freebuildmode then
                --V2C: free-build should still trigger prototyping
                if not table.contains(self.recipes, ing_recipe.name) and
                    CanPrototypeRecipe(ing_recipe.level, self.accessible_tech_trees) then
