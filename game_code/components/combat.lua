...
local function onpanicthresh(self, panicthresh)
  self.inst.replica.combat:SetIsPanic(panicthresh ~= nil and self.inst.components.health ~= nil and
    panicthresh > self.inst.components.health:GetPercent())
end
...
function Combat:GetCooldown()
  return self.laststartattacktime ~= nil and math.max(0, self.min_attack_period - GetTime() + self.laststartattacktime)
      or 0
end
...
function Combat:OnEntityWake()
  ...
  if self.retargetperiod ~= nil then
    self.retargettask = self.inst:DoPeriodicTask(self.retargetperiod, dotryretarget, self.retargetperiod * math.random()
      , self)
  end
...
function Combat:EngageTarget(target)
  if target then
    if not
        (
        self.inst.components.follower and self.inst.components.follower.leader == target and
            self.inst.components.follower.leader.components.leader ~= nil and
            self.inst.components.follower.keepleaderonattacked) then
      local oldtarget = self.target
      self.target = target
      ...
      if self.inst.components.follower and self.inst.components.follower.leader == target and
          self.inst.components.follower.leader.components.leader then
...
function Combat:ShouldAggro(target, ignore_forbidden)
  if target ~= nil and
      ...
      (
      target.components.combat == nil or target.components.combat.shouldavoidaggrofn == nil or
          target.components.combat.shouldavoidaggrofn(self.inst, target))
  then
...
function Combat:GetAttacked(attacker, damage, weapon, stimuli)
  ...
  if self.inst.components.health ~= nil and damage ~= nil and damageredirecttarget == nil then
    ...
    if damage > 0 and not self.inst.components.health:IsInvincible() then
      ...
      damageresolved = self.inst.components.health:DoDelta(-damage, nil,
        cause ~= nil and (cause.nameoverride or cause.prefab) or "NIL", nil, cause)
  ...
  if not blocked then
    self.inst:PushEvent("attacked",
      { attacker = attacker, damage = damage, damageresolved = damageresolved, original_damage = original_damage,
        weapon = weapon, stimuli = stimuli, redirected = damageredirecttarget, noimpactsound = self.noimpactsound })
    ...
    if attacker ~= nil then
      attacker:PushEvent("onhitother",
        { target = self.inst, damage = damage, damageresolved = damageresolved, stimuli = stimuli, weapon = weapon,
...
function Combat:GetImpactSound(target, weapon)
  ...
  elseif target:HasTag("object") then
    ...
    ((target:HasTag("hive") or target:HasTag("eyeturret") or target:HasTag("houndmound")) and "hive_") or
        ...
        ((target:HasTag("shadow") or target:HasTag("shadowminion") or target:HasTag("shadowchesspiece")) and "shadow_")
        or
    ...
    return hitsound .. (
        ...
        ((target:HasTag("smallcreature") or target:HasTag("small")) and "sml_") or
            (
            (target:HasTag("largecreature") or target:HasTag("epic") or target:HasTag("large")) and
                not (target:HasTag("shadowchesspiece") or target:HasTag("fossil")) and "lrg_") or
...
function Combat:LocomotorCanAttack(reached_dest, target)
  ...
  if attackrange > 2 * 2 and self.inst:HasTag("player") then
    ...
    if not is_ranged_weapon then
      ...
      if TheWorld.Map:IsNotValidGroundAtPoint(voidtest:Get()) and
          not TheWorld.Map:IsNotValidGroundAtPoint(target.Transform:GetWorldPosition()) then
        reached_dest = false
      end
    end
  end
...
function Combat:GetHitRange()
  local weapon = self:GetWeapon()
  return self.temprange or
      weapon ~= nil and weapon.components.weapon.hitrange ~= nil and self.hitrange + weapon.components.weapon.hitrange or
      self.hitrange
end
...
function Combat:DoAttack(targ, weapon, projectile, stimuli, instancemult, instrangeoverride, instpos)
  ...
  if targ.components.combat ~= nil then
    ...
    (stimuli == "electric" or
        ...
        (
        targ.components.moisture ~= nil and targ.components.moisture:GetMoisturePercent() or
            (targ:GetIsWet() and 1 or 0
            ))
        or 1
