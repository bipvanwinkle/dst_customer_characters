...
function Sanity:GetDebugString()
  return string.format("%2.2f / %2.2f at %2.4f. Penalty %2.2f, Mode %s", self.current, self.max, self.rate, self.penalty
    , self.mode == SANITY_MODE_INSANITY and "INSANITY" or "LUNACY")
end
...
function Sanity:DoDelta(delta, overtime)
  ...
  self.inst:PushEvent("sanitydelta",
    { oldpercent = self._oldpercent, newpercent = self:GetPercent(), overtime = overtime, sanitymode = self.mode })
...
function Sanity:Recalc(dt)
  ...
  if self.dapperness_mult ~= 0 then
    ...
    for k, v in pairs(self.inst.components.inventory.equipslots) do
      ...
      if equippable ~= nil then
        local item_dapperness = self.get_equippable_dappernessfn ~= nil and
            self.get_equippable_dappernessfn(self.inst, equippable) or
            equippable:GetDapperness(self.inst, self.no_moisture_penalty)
  ...
  local moisture_delta = self.no_moisture_penalty and 0 or
      easing.inSine(self.inst.components.moisture:GetMoisture(), 0, TUNING.MOISTURE_SANITY_PENALTY_MAX,
    ...
    for i, v in ipairs(ents) do
      if v.components.sanityaura ~= nil and v ~= self.inst then
        ...
        if not is_aura_immune then
          ...
          aura_val = (
              aura_val < 0 and
                  (self.neg_aura_absorb > 0 and self.neg_aura_absorb * -aura_val or aura_val) * self:GetAuraMultipliers()
                  or aura_val)
  ...
  if mount ~= nil and mount.components.sanityaura ~= nil then
    ...
    aura_val = (
        aura_val < 0 and
            (self.neg_aura_absorb > 0 and self.neg_aura_absorb * -aura_val or aura_val) * self:GetAuraMultipliers() or
            aura_val)
    aura_delta = aura_delta + ((aura_val < 0 and self.neg_aura_immune) and 0 or aura_val)
  end
