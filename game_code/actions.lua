...
local function CheckFishingOceanRange(doer, dest)
  ...
  if TheWorld.Map:IsVisualGroundAtPoint(test_pt.x, 0, test_pt.z) or
      TheWorld.Map:GetPlatformAtPoint(test_pt.x, test_pt.z) ~= nil then
    return false
  else
    return true
  end
...
local function ExtraDeployDist(doer, dest, bufferedaction)
  if dest ~= nil then
    ...
    if is_on_water then
      return (
          (
              bufferedaction ~= nil and bufferedaction.invobject ~= nil and
                  bufferedaction.invobject:HasTag("usedeployspacingasoffset") and
...
Action = Class(function(self, data, instant, rmb, distance, ghost_valid, ghost_exclusive, canforce, rangecheckfn)
  ...
  if type(data) ~= "table" then
    ...
    local priority = data
    data = { priority = priority, instant = instant, rmb = rmb, ghost_valid = ghost_valid,
      ghost_exclusive = ghost_exclusive, canforce = canforce, rangecheckfn = rangecheckfn }
  end
...
ACTIONS =
{
  ...
  DROP = Action({ priority = -1, mount_valid = true, encumbered_valid = true, is_relative_to_platform = true,
    extra_arrive_dist = ExtraDropDist }),
  TRAVEL = Action(),
  ...
  FISH_OCEAN = Action({ rmb = true, customarrivecheck = CheckFishingOceanRange, is_relative_to_platform = true,
    disable_platform_hopping = true }),
  FISH = Action(),
  REEL = Action({ instant = true }),
  OCEAN_FISHING_POND = Action(),
  OCEAN_FISHING_CAST = Action({ priority = 3, rmb = true, customarrivecheck = CheckOceanFishingCastRange,
    is_relative_to_platform = true, disable_platform_hopping = true }),
  ...
  BLINK_MAP = Action({ priority = HIGH_ACTION_PRIORITY, customarrivecheck = ArriveAnywhere, rmb = true,
    mount_valid = true,
    map_action = true, }),
  ...
  HAUNT = Action({ rmb = false, mindistance = 2, ghost_valid = true, ghost_exclusive = true, canforce = true,
  ...
  WATER_TOSS = Action({ priority = 3, rmb = true, customarrivecheck = CheckOceanFishingCastRange,
    is_relative_to_platform = true, disable_platform_hopping = true }),
  ...
  CAST_NET = Action({ priority = HIGH_ACTION_PRIORITY, rmb = true, distance = 12, mount_valid = true,
    disable_platform_hopping = true }),
  ROW_FAIL = Action({ customarrivecheck = ArriveAnywhere, disable_platform_hopping = true, skip_locomotor_facing = true,
    invalid_hold_action = true }),
  ROW = Action({ priority = 3, customarrivecheck = CheckRowRange, is_relative_to_platform = true,
    disable_platform_hopping = true, invalid_hold_action = true }),
  ROW_CONTROLLER = Action({ priority = 3, is_relative_to_platform = true, disable_platform_hopping = true,
    do_not_locomote = true, invalid_hold_action = true }),
  BOARDPLATFORM = Action({ customarrivecheck = CheckIsOnPlatform }),
  OCEAN_TOSS = Action({ priority = 3, rmb = true, customarrivecheck = CheckOceanFishingCastRange,
    is_relative_to_platform = true, disable_platform_hopping = true }),
  UNPATCH = Action({ distance = 0.5 }),
  POUR_WATER = Action({ rmb = true, tile_placer = "gridplacer", show_tile_placer_fn = ShowPourWaterTilePlacer,
    extra_arrive_dist = ExtraPourWaterDist }),
  POUR_WATER_GROUNDTILE = Action({ rmb = true, customarrivecheck = CheckTileWithinRange, tile_placer = "gridplacer",
    theme_music = "farming" }),
  ...
  LIFT_GYM_SUCCEED_PERFECT = Action({ do_not_locomote = true, disable_platform_hopping = true,
...
ACTIONS.STEAL.fn = function(act)
  local owner = act.target.components.inventory ~= nil and act.target or
      act.target.components.inventoryitem ~= nil and act.target.components.inventoryitem.owner or nil
    ...
    if targ.components.inspectable ~= nil then
      ...
      if desc ~= nil then
        ...
        if act.doer.components.talker ~= nil then
          act.doer.components.talker:Say(desc, nil, targ.components.inspectable.noanim, nil, nil, nil,
            text_filter_context, original_author)
        end
        return true
      end
    end
...
ACTIONS.OCEAN_FISHING_CAST.fn = function(act)
  ...
  return (rod ~= nil and rod.components.oceanfishingrod ~= nil) and rod.components.oceanfishingrod:Cast(act.doer, pos) or
      nil
end

ACTIONS.OCEAN_FISHING_REEL.strfn = function(act)
  local rod = act.invobject or
      act.doer.replica.inventory ~= nil and act.doer.replica.inventory:GetEquippedItem(EQUIPSLOTS.HANDS) or nil
  local target = (rod ~= nil and rod:IsValid() and rod.replica.oceanfishingrod ~= nil) and
      rod.replica.oceanfishingrod:GetTarget() or nil
...
ACTIONS.CHANGE_TACKLE.strfn = function(act)
  ...
  local equipped = (item ~= nil and act.doer.replica.inventory ~= nil) and
      act.doer.replica.inventory:GetEquippedItem(EQUIPSLOTS.HANDS) or nil
  return (equipped ~= nil and equipped.replica.container ~= nil and equipped.replica.container:IsHolding(item)) and
      "REMOVE"
...
ACTIONS.CHANGE_TACKLE.fn = function(act)
  ...
  if act.invobject.components.inventoryitem:IsHeldBy(equipped) then
    ...
    if cur_item == nil then
      ...
      if equipped.components.container.acceptsstacks and act.invobject.prefab == cur_item.prefab and
          act.invobject.skinname == cur_item.skinname
        ...
        return true
      elseif (
          act.invobject.prefab ~= cur_item.prefab and
              (act.invobject.skinname == nil or act.invobject.skinname ~= cur_item.skinname)) or
          cur_item.components.perishable then
        local item = act.invobject.components.inventoryitem:RemoveFromOwner(equipped.components.container.acceptsstacks)
...
ACTIONS.DEPLOY.fn = function(act)
  ...
  if act.invobject ~= nil and act.invobject.components.deployable ~= nil and
      act.invobject.components.deployable:CanDeploy(act_pos, nil, act.doer, act.rotation) then
...
local function ValidToolWork(act, workaction)
  return act.target.components.workable ~= nil and act.target.components.workable:CanBeWorked() and
      act.target.components.workable:GetWorkAction() == workaction
end
  ...
  if act.invobject ~= nil and act.invobject.components.fertilizer ~= nil then
    ...
    if not (act.doer ~= nil and act.doer.components.rider ~= nil and act.doer.components.rider:IsRiding()) then
      if act.target ~= nil then
        if act.target.components.crop ~= nil and
            not (act.target.components.crop:IsReadyForHarvest() or act.target:HasTag("withered")) then
...
ACTIONS.FILL.fn = function(act)
  ...
  if groundpt ~= nil then
    local success = filled_object.components.fillable.acceptsoceanwater and
        TheWorld.Map:IsOceanAtPoint(groundpt.x, 0, groundpt.z)
    if success then
...
ACTIONS.CARNIVALGAME_FEED.fn = function(act)
  if act.invobject ~= nil and act.invobject:IsValid() and act.target ~= nil and act.target:IsValid() and
      act.target.components.carnivalgamefeedable ~= nil then
...
ACTIONS.STORE.fn = function(act)
  ...
  if target.components.container == nil and target.components.constructionsite ~= nil then
    ...
    target = builder == act.doer and builder.components.constructionbuilder ~= nil and
        builder.components.constructionbuilder.constructioninst or nil
    if target == nil then
      return false
    end
  end
  --
  if target.components.container ~= nil and act.invobject.components.inventoryitem ~= nil and
    ...
    if act.doer.components.constructionbuilderuidata ~= nil and
        act.doer.components.constructionbuilderuidata:GetContainer() == target then
...
ACTIONS.STORE.strfn = function(act)
  if act.target ~= nil then
    return (
        (act.target:HasTag("stewer") or act.target:HasTag("quagmire_stewer")) and
            (act.target:HasTag("spicer") and "SPICE" or "COOK"))
...
ACTIONS.EXTINGUISH.fn = function(act)
  if act.target.components.burnable ~= nil and act.target.components.burnable:IsBurning() then
    ...
    return true
  elseif act.target.components.fueled ~= nil and act.target.components.fueled.canbespecialextinguished and
      not act.target.components.fueled:IsEmpty() then
    act.target.components.fueled:ChangeSection(-1)
  end
end
...
ACTIONS.INVESTIGATE.fn = function(act)
  local investigatePos = act.doer.components.knownlocations ~= nil and
      act.doer.components.knownlocations:GetLocation("investigate") or nil
...
ACTIONS.ACTIVATE.fn = function(act)
  if act.target.components.activatable ~= nil and
      (
      act.target.components.burnable == nil or
          not (act.target.components.burnable:IsSmoldering() or act.target.components.burnable:IsBurning())) and
      act.target.components.activatable:CanActivate(act.doer) then
...
ACTIONS.HEAL.fn = function(act)
  ...
  if target ~= nil and act.invobject ~= nil and target.components.health ~= nil and
...
ACTIONS.BLINK.strfn = function(act)
  return act.invobject == nil and act.doer ~= nil and act.doer:HasTag("soulstealer") and
      ((act.doer._freesoulhop_counter or 0) > 0 and "FREESOUL" or "SOUL") or nil
end
...
ACTIONS.BLINK_MAP.stroverridefn = function(act)
  return act.invobject == nil and act.doer ~= nil and act.doer:HasTag("soulstealer") and
      subfmt(STRINGS.ACTIONS.BLINK_MAP.SOUL, { souls = act.distancecount, }) or nil
end
...
ACTIONS_MAP_REMAP[ACTIONS.BLINK.code] = function(act, targetpos)
  ...
  if not TheWorld.Map:IsVisualGroundAtPoint(targetpos.x, targetpos.y, targetpos.z) then
    ...
    local revealer = nil
    local MAX_WALKABLE_PLATFORM_DIAMETERSQ = TUNING.MAX_WALKABLE_PLATFORM_RADIUS * TUNING.MAX_WALKABLE_PLATFORM_RADIUS *
        4 -- Diameter.
...
ACTIONS.COMBINESTACK.fn = function(act)
  ...
  if invobj and target and invobj.prefab == target.prefab and invobj.skinname == target.skinname and
      target.components.stackable and not target.components.stackable:IsFull() then
...
ACTIONS.CATPLAYGROUND.fn = function(act)
  if act.doer then
    if act.target then
      ...
      elseif act.doer.components.combat ~= nil and math.random() < TUNING.CATCOON_ATTACK_CONNECT_CHANCE and
          act.target.components.health and act.target.components.health.maxhealth <= TUNING.PENGUIN_HEALTH
          -- Only bother attacking if it's a penguin or weaker
      ...
      elseif act.doer.components.inventory ~= nil and act.target.components.inventoryitem and
          act.target.components.inventoryitem.canbepickedup and math.random() < TUNING.CATCOON_PICKUP_ITEM_CHANCE then
      ...
      elseif act.target.components.activatable ~= nil and act.target.components.activatable:CanActivate(act.doer) and
          math.random() < TUNING.CATCOON_ACTIVATE_CONNECT_CHANCE then
...
ACTIONS.CATPLAYAIR.fn = function(act)
  if act.doer and act.target then
    ...
    elseif act.doer.components.combat ~= nil and act.target and math.random() < TUNING.CATCOON_ATTACK_CONNECT_CHANCE
        and act.target.components.health and act.target.components.health.maxhealth <= TUNING.PENGUIN_HEALTH
        -- Only bother attacking if it's a penguin or weaker
        and act.target.components.combat and act.target.components.combat:CanBeAttacked(act.doer)
...
ACTIONS.TOSS.fn = function(act)
  if act.invobject and act.doer then
    if act.invobject.components.complexprojectile and act.doer.components.inventory and
        (act.invobject.components.equippable == nil or not act.invobject.components.equippable:IsRestricted(act.doer)) then
...
ACTIONS.RETURN_FOLLOWER.fn = function(act)
  if act.target ~= nil and act.target.components.follower ~= nil and
      act.target.components.follower:GetLeader() == act.doer and act.doer:HasTag("near_kitcoonden") then
...
ACTIONS.CASTAOE.fn = function(act)
  local act_pos = act:GetActionPoint()
  if act.invobject ~= nil and act.invobject.components.aoespell ~= nil and
      act.invobject.components.aoespell:CanCast(act.doer, act_pos) then
  ...
  if act.target ~= nil and
    ...
    if act.target.components.container ~= nil then
      if act.target.components.container:IsOpen() then
        return false, "INUSE"
      elseif not act.target.components.container:IsEmpty() or
          (act.target.components.stewer ~= nil and act.target.components.stewer:IsDone()) then
...
ACTIONS.HALLOWEENMOONMUTATE.fn = function(act)
  if act.invobject ~= nil and act.invobject.components.halloweenpotionmoon ~= nil then
    if act.target == nil
        ...
        or
        (
        act.target.components.burnable ~= nil and
            (act.target.components.burnable:IsBurning() or act.target.components.burnable:IsSmoldering()))
...
ACTIONS.START_CARRAT_RACE.fn = function(act)
  if act.target ~= nil and act.target.components.yotc_racestart ~= nil and
      act.target.components.yotc_racestart:CanInteract() and
      not (act.target:HasTag("fire") or act.target:HasTag("burnt")) then

    local race_data = TheWorld.components.yotc_raceprizemanager ~= nil and
        TheWorld.components.yotc_raceprizemanager:GetRaceById(act.target) or nil
    ...
    if act.doer and act.target.racestartstring then
      act.doer:DoTaskInTime(2,
        function() if act.target:HasTag("race_on") then act.doer.components.talker:Say(GetString(act.doer,
              act.target.racestartstring))
          end
        end)
    end
    return true
...
ACTIONS.REPLATE.stroverridefn = function(act)
  ...
  local replatable_inst = (act.target ~= nil and act.target:HasTag("quagmire_replatable") and act.target) or
      (act.invobject ~= nil and act.invobject:HasTag("quagmire_replatable") and act.invobject) or nil
...
ACTIONS.LOWER_SAIL_BOOST.fn = function(act)
  if act.target ~= nil and act.target.components.mast ~= nil and act.target.components.mast.is_sail_raised then
    ...
    local strength = (
        act.doer.components.expertsailor ~= nil and act.doer.components.expertsailor:GetLowerSailStrength() or
            TUNING.DEFAULT_SAIL_BOOST_STRENGTH) * (act.doer:HasTag("master_crewman") and 2 or 1)
...
ACTIONS.REPAIR_LEAK.fn = function(act)
  if act.invobject ~= nil and act.target ~= nil and act.target.components.boatleak ~= nil and
      act.target:HasTag("boat_leak") then
...
ACTIONS.BOAT_CANNON_LOAD_AMMO.fn = function(act)
  if act.target.components.boatcannon ~= nil and not act.target.components.boatcannon:IsAmmoLoaded() and
      act.doer.components.inventory ~= nil then
    ...
    if replater ~= nil and replatable ~= nil and replater ~= replatable then
      ...
      if owner ~= nil then
        ...
        elseif replatable.inst == act.invobject then
          ...
          local perish = replatable.inst.components.perishable ~= nil and
              replatable.inst.components.perishable:GetPercent() or nil
...
ACTIONS.SALT.fn = function(act)
  if act.target ~= nil and act.target.components.quagmire_saltable ~= nil then
    ...
    if owner ~= nil then
      ...
      local replated = act.target.components.quagmire_replatable ~= nil and
          act.target.components.quagmire_replatable.dishtype or nil
      act.target:Remove()
      local newitem = SpawnPrefab(prefab)
      if perish ~= nil then
        newitem.components.perishable:SetPercent(perish)
...
ACTIONS.PLANTREGISTRY_RESEARCH.fn = function(act)
  ...
  if targ ~= nil then
    if targ.components.plantresearchable then
      if targ.components.plantresearchable:IsRandomSeed() then
        if act.doer.components.talker then
          act.doer.components.talker:Say(GetString(act.doer, "ANNOUNCE_PLANT_RANDOMSEED"), nil,
            targ.components.inspectable.noanim)
        end
      else
        targ.components.plantresearchable:LearnPlant(act.doer)

        if act.doer.components.talker then
          act.doer.components.talker:Say(GetString(act.doer, "ANNOUNCE_PLANT_RESEARCHED"), nil,
            targ.components.inspectable.noanim)
        end
      end
    elseif targ.components.fertilizerresearchable then
      ...
      if act.doer.components.talker then
        act.doer.components.talker:Say(GetString(act.doer, "ANNOUNCE_FERTILIZER_RESEARCHED"), nil,
          targ.components.inspectable.noanim)
      end
    end
    return true
...
ACTIONS.USE_HEAVY_OBSTACLE.fn = function(act)
  ...
  if heavy_item == nil or not act.target:HasTag("can_use_heavy")
      or
      (
      act.target.use_heavy_obstacle_action_filter ~= nil and
          not act.target.use_heavy_obstacle_action_filter(act.target, act.doer, heavy_item)) then
